...
description: Use when the user asks for a rich explanation of a code change, diff, branch, or PR. Produces an interactive HTML page, shared as a gist.
---
...
- Output a single self-contained HTML file which includes CSS and JavaScript, so it opens with no network. Make the whole thing one long page with section headers and a table of contents. Don't use tabs for the top-level structure. Basic responsive styling so you can view it on a phone is nice too.
- Save it outside the code repo, with the filename starting with today's date in `YYYY-MM-DD-` format so the files stay time-sorted. Then publish it and give me the rendered link:

  ```
  gh gist create /tmp/2026-01-12-explanation-<slug>.html --desc "Explainer: <what it explains>"
  ```

  GitHub shows gist HTML as source, so send me `https://gistpreview.github.io/?<gist-id>`, which renders it.
